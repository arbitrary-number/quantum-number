...
bitwiseMultiply PROC
    ...
    mov rdi, 64     ; 64-bit counter

bitwise_mult_loop:
    test rsi, 1     ; Check if LSB of multiplier is 1
    jz bitwise_mult_shift

    ; Add current multiplicand value to result
    add rax, rbx

bitwise_mult_shift:
    ; Shift multiplier right for next bit
    shr rsi, 1
    ; Shift multiplicand left for next power of 2
    shl rbx, 1
...
quantumMultiply PROC
    ...
    mov rsi, rcx    ; Save pointer to quantum number 1
    mov rdi, rdx    ; Save pointer to quantum number 2

    ; Display operation
    push rcx        ; Save RCX (pointer to quantum number 1)
    lea rcx, [msgMultiply]
    call printString
    mov rcx, QWORD PTR [rsi + 32]    ; First number
    call printQWORD
    lea rcx, [msgMultiply]
    call printString
...
handle_mult_overflow:
    ...
    mov QWORD PTR [rsi + RIGHT_PTR_OFFSET], rax  ; Set rightPtr to overflow node
...
physicsChemistryDemo PROC
    ...
    ; Demonstrate physics operations - Schrodinger equation simulation
    lea rcx, [msgSchrodingerEquation]
    call printString
    lea rcx, [msgNewline]
    call printString

    ; Demonstrate chemistry operations - molecular orbital theory
    lea rcx, [msgMolecularOrbital]
    call printString
    lea rcx, [msgNewline]
    call printString

    ; Demonstrate quantum chemistry calculations using polynomial operations
    ; Initialize molecular orbital coefficients: 1 + 2x + 3x² (representing atomic orbitals)
    mov QWORD PTR [nodeRoot + 32], 1    ; s-orbital coefficient
    mov QWORD PTR [nodeRoot + 40], 2    ; p-orbital coefficient
    mov QWORD PTR [nodeRoot + 48], 3    ; d-orbital coefficient

    ; Display molecular orbital calculation
    lea rcx, [msgMolecularOrbitalCalc]
    call printString
    mov rcx, QWORD PTR [nodeRoot + 32]
    call printQWORD
    lea rcx, [msgPlus]
...
msgSchrodingerEquation    db "SCHRODINGER EQUATION: -h²/2m d²ψ/dx² + Vψ = Eψ", 0
msgMolecularOrbital      db "MOLECULAR ORBITAL THEORY: ψ = Σ c_i φ_i", 0
msgMolecularOrbitalCalc  db "MOLECULAR ORBITAL CALCULATION: ", 0
