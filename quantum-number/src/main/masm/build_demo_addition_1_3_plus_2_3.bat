...
REM echo Locating kernel32.lib...
REM set "KERNEL32_LIB="
REM for /f "delims=" %%K in ('dir /s /b "C:\Program Files (x86)\Windows Kits\10\Lib" ^| findstr /i kernel32.lib') do (
REM     set "KERNEL32_LIB=%%K"
REM     goto :found_kernel32
REM )

REM Locate x64 version of kernel32.lib
for /f "delims=" %%K in (
    'dir /s /b "C:\Program Files (x86)\Windows Kits\10\Lib" ^| findstr /i /c:"um\\x64\\kernel32.lib"'
) do (
    set "KERNEL32_LIB=%%K"
    goto :found_kernel32
)
